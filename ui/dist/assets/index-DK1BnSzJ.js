...
`),e=new Blob([i],{type:"text/plain"}),t=URL.createObjectURL(e),s=document.createElement("a");s.href=t,s.download="activity.log",s.click(),URL.revokeObjectURL(t)}toggleFullscreen(){var i,e;this.fullscreen?(e=document.exitFullscreen)==null||e.call(document):(i=this.requestFullscreen)==null||i.call(this)}firstUpdated(){document.addEventListener("fullscreenchange",this.handleFullscreenChange)}disconnectedCallback(){this.disconnect(),document.removeEventListener("fullscreenchange",this.handleFullscreenChange),super.disconnectedCallback()}renderLine(i){return g`<pre class="log-line">${i}</pre>`}render(){return g`
  ...
  `;ms([w()],ri.prototype,"logs",2);ms([w()],ri.prototype,"error",2);ms([w()],ri.prototype,"loading",2);ms([w()],ri.prototype,"fullscreen",2);ms([w()],ri.prototype,"connected",2);ri=ms([be("esp-activity-log-page")],ri);var nw=Object.defineProperty,rw=Object.getOwnPropertyDescriptor,xe=(i,e,t,s)=>{for(var n=s>1?void 0:s?rw(e,t):e,r=i.length-1,o;r>=0;r--)(o=i[r])&&(n=(s?o(e,t,n):o(n))||n);return s&&n&&nw(e,t,n),n};let fe=class extends re{constructor(){super(...arguments),this.step1="scanning",this.step2="disabled",this.step3="disabled",this.discoveredBridges=[],this.bridgeError=null,this.manualMode=!1,this.manualHost="",this.manualPort=80,this.manualApiKey="",this.apiKeyInput="",this.selectedBridge=null,this.restartError=null,this.integrationError=null,this.statusPollTimer=null,this.integrationPollTimer=null,this.integrationFailures=0,this.pollingSeconds=0,this.discoveryTimer=null,this.doneTimer=null,this.validatePollTimer=null,this.validateAttempts=0,this.activeBridgeUuid=null,this.bridgeHost=null,this.bridgePort=80}connectedCallback(){super.connectedCallback(),this.initFromBridgeConfig()}async initFromBridgeConfig(){const i=await S.setupStatus(),e=i.integration.configured;i.bridge.configured?(this.activeBridgeUuid=i.bridge.uuid||null,this.bridgeHost=i.bridge.ip||null,i.bridge.connected?(this.step1="complete",this.startConfiguredBridgeFlow(i,e)):(this.step1="pending",this.validateAttempts=0,this.discoveryTimer&&(clearInterval(this.discoveryTimer),this.discoveryTimer=null),this.activeBridgeUuid&&S.bridgeReconnect(this.activeBridgeUuid),this.validatePollTimer=setInterval(()=>void this.pollValidateStatus(),1e3))):(this.step2!=="disabled"&&(this.step2="disabled"),this.step3!=="disabled"&&(this.step3="disabled"),this.startDiscovery(),this.statusPollTimer=setInterval(()=>void this.pollStatus(),3e3))}async startConfiguredBridgeFlow(i,e){i.restart.required?["restarting","polling","complete"].includes(this.step2)||(this.step2="ready"):(this.step2="complete",!e&&this.step3==="disabled"&&this.triggerIntegrationSetup()),this.statusPollTimer=setInterval(()=>void this.pollStatus(),3e3)}async pollValidateStatus(){if(this.step1!=="pending"){this.validatePollTimer&&(clearInterval(this.validatePollTimer),this.validatePollTimer=null);return}this.validateAttempts++;try{const i=await S.setupStatus();if(i.bridge.connected){this.step1="complete",this.validatePollTimer&&(clearInterval(this.validatePollTimer),this.validatePollTimer=null),this.startConfiguredBridgeFlow(i,i.integration.configured);return}}catch{}this.validateAttempts>=3&&(this.validatePollTimer&&(clearInterval(this.validatePollTimer),this.validatePollTimer=null),this.step1="error",this.startDiscovery(),this.statusPollTimer=setInterval(()=>void this.pollStatus(),3e3))}disconnectedCallback(){this.statusPollTimer&&(clearInterval(this.statusPollTimer),this.statusPollTimer=null),this.integrationPollTimer&&(clearInterval(this.integrationPollTimer),this.integrationPollTimer=null),this.discoveryTimer&&(clearInterval(this.discoveryTimer),this.discoveryTimer=null),this.validatePollTimer&&(clearInterval(this.validatePollTimer),this.validatePollTimer=null),this.doneTimer&&(clearTimeout(this.doneTimer),this.doneTimer=null),super.disconnectedCallback()}async startDiscovery(){this.discoveryTimer&&(clearInterval(this.discoveryTimer),this.discoveryTimer=null),this.step1!=="complete"&&(this.step1="scanning"),this.bridgeError=null,await this.refreshDiscoveredBridges(),S.triggerScan().catch(i=>{const e=i instanceof Error?i.message:String(i);e.toLowerCase().includes("already")||(this.bridgeError=e)}),this.discoveryTimer=setInterval(()=>void this.refreshDiscoveredBridges(),2e3)}async refreshDiscoveredBridges(){if(!(this.step1==="complete"||this.step1==="connecting"||this.step1==="pending"))try{this.discoveredBridges=await S.discoverBridges(),this.discoveredBridges.length>0?(this.step1="found",this.bridgeError=null):this.step1!=="error"&&(this.step1="scanning")}catch(i){this.bridgeError=i instanceof Error?i.message:String(i)}}async pollStatus(){try{const i=await S.setupStatus(),e=i.integration.configured;i.bridge.configured&&this.step1!=="pending"?(this.step1="complete",this.discoveryTimer&&(clearInterval(this.discoveryTimer),this.discoveryTimer=null),i.restart.required?["restarting","polling","complete"].includes(this.step2)||(this.step2="ready"):(this.step2="complete",!e&&this.step3==="disabled"&&this.triggerIntegrationSetup())):(this.step2!=="disabled"&&(this.step2="disabled"),this.step3!=="disabled"&&(this.step3="disabled")),i.bridge.configured&&!i.restart.required&&this.step2==="polling"&&(this.step2="complete",this.pollingSeconds=0,!e&&this.step3==="disabled"&&this.triggerIntegrationSetup()),e&&(this.step3="complete",this.integrationFailures=0,this.step1==="complete"&&this.step2==="complete"&&this.onAllDone()),this.step2==="polling"&&(this.pollingSeconds+=3)}catch{}}async connectBridgeBySelect(i){if(!this.apiKeyInput.trim()){this.bridgeError="API key is required";return}this.step1="connecting",this.bridgeError=null;try{await S.selectBridge(i.host,i.port,i.name,i.version,this.apiKeyInput,i.network_id,i.hostname),this.step1="complete",this.step2="ready",this.pollStatus()}catch(e){this.step1="found",this.bridgeError=e instanceof Error?e.message:String(e)}}async connectManualBridge(){if(!this.manualHost.trim()){this.bridgeError="Host is required";return}this.step1="connecting",this.bridgeError=null;try{await S.addBridge(this.manualHost.trim(),this.manualPort,void 0,this.manualApiKey||"",""),this.step1="complete",this.step2="ready",this.pollStatus()}catch(i){this.step1=this.discoveredBridges.length>0?"found":"error",this.bridgeError=i instanceof Error?i.message:String(i)}}selectBridgeForApiKey(i){this.selectedBridge=i,this.discoveryTimer&&(clearInterval(this.discoveryTimer),this.discoveryTimer=null)}async handleRestart(){this.step2="restarting",this.restartError=null;try{const i=await S.requestRestart();i.success?(this.step2="polling",this.pollingSeconds=0):(this.step2="error",this.restartError=i.error||"Restart failed")}catch{this.step2="polling",this.pollingSeconds=0,this.restartError=null}}async triggerIntegrationSetup(){if(!["triggering","polling","complete"].includes(this.step3)){this.step3="triggering",this.integrationError=null,this.integrationFailures=0;try{const i=await S.integrationSetup();if(i.entry_created){this.step3="complete",this.onAllDone();return}i.success?(this.step3="polling",this.integrationPollTimer||(this.integrationPollTimer=setInterval(()=>void this.pollIntegrationForEntry(),3e3))):(this.step3="error",this.integrationError=i.error||"Failed to set up integration")}catch(i){this.step3="error",this.integrationError=i instanceof Error?i.message:String(i)}}}async pollIntegrationForEntry(){this.integrationFailures++;try{if((await S.setupStatus()).integration.configured){this.step3="complete",this.integrationPollTimer&&(clearInterval(this.integrationPollTimer),this.integrationPollTimer=null),this.onAllDone();return}}catch{}this.integrationFailures>=10&&(this.integrationPollTimer&&(clearInterval(this.integrationPollTimer),this.integrationPollTimer=null),this.step3="fallback")}async onAllDone(){this.doneTimer||(this.doneTimer=setTimeout(()=>{this.doneTimer=null,window.location.hash="/"},2e3))}dismiss(){this.doneTimer&&(clearTimeout(this.doneTimer),this.doneTimer=null),this.dispatchEvent(new CustomEvent("setup-dismissed",{bubbles:!0,composed:!0})),window.location.hash="/"}retryDiscovery(){this.startDiscovery()}retryConnection(){this.step1="pending",this.validateAttempts=0,this.activeBridgeUuid&&S.bridgeReconnect(this.activeBridgeUuid),this.statusPollTimer&&(clearInterval(this.statusPollTimer),this.statusPollTimer=null),this.validatePollTimer=setInterval(()=>void this.pollValidateStatus(),1e3)}render(){return g`
...
//# sourceMappingURL=index-DK1BnSzJ.js.map
