...
class IntegrationWsManager:
    ...
    def start(self) -> None:
        if self._task is None or self._task.done():
            ...
            self._task = asyncio.create_task(self._run(), name="espnow-tree-integration-ws")
    ...
    async def refresh_once(self) -> dict[str, Any]:
        result = await self._api.call({"type": "espnow_tree/topology"}, timeout=15.0)
    ...
    async def send_config(self, mac: str, command: str, params: dict[str, Any] | None = None) -> dict[str, Any]:
        result = await self._api.call(
            {
                "type": "espnow_tree/config_command",
                "mac": mac,
                "command": command,
                "params": params or {},
            },
