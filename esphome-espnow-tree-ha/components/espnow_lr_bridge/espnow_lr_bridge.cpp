...
bool ESPNowLRBridge::init_wifi_and_espnow_() {
  ...
  protocols.ghz_2g = (espnow_mode_ == "lr") ? WIFI_PROTOCOL_LR : WIFI_PROTOCOL_11N;
...
void ESPNowLRBridge::queue_availability_(const uint8_t *mac, bool online, const char *reason) {
  ...
  if (api_ws_ != nullptr) {
    ...
    api_ws_->emit_remote_availability(mac, online, reason, rssi, last_seen_ms, parent_mac, hop_count);
  }
  if (online) {
...
void ESPNowLRBridge::log_airtime_status_() {
  ...
  ESP_LOGI(TAG, "up:%s | radio(LR):%.1f%% | tx:%u rx:%u | rssi:%ddBm | fail:%d%% | children:%u",
           format_uptime(uptime_s).c_str(), busy_pct, total_tx, total_rx, avg_rssi, static_cast<int>(fail_pct), total_children);
...
std::string ESPNowLRBridge::api_topology_snapshot_json(const std::string &request_payload_json) const {
  ...
  json += "\"hop_count\":0,";
  json += "\"rssi\":\"N.A.\",";
  json += "\"last_seen_ms\":" + std::to_string(now_ms) + ",";
  ...
  json += "\"build_date\":\"" + bridge_api::BridgeApiMessages::escape_json(protocol_.bridge_identity().build_date) + "\",";
  json += "\"build_time\":\"" + bridge_api::BridgeApiMessages::escape_json(protocol_.bridge_identity().build_time) + "\",";
  ...
  for (const auto &entry : protocol_.get_sessions()) {
    ...
    json += "\"build_date\":\"";
    if (session.build_date.empty()) json += "unknown"; else json += bridge_api::BridgeApiMessages::escape_json(session.build_date);
    json += "\",\"build_time\":\"";
    if (session.build_time.empty()) json += "unknown"; else json += bridge_api::BridgeApiMessages::escape_json(session.build_time);
    json += "\",";
...
void ESPNowLRBridge::register_ota_web_handlers_() {
  ...
  struct OtaStatusHandler : public AsyncWebHandler {
    ...
    void handleRequest(AsyncWebServerRequest *request) override {
      ...
      for (uint32_t sequence : (bridge_->ota_manager_ != nullptr ? bridge_->ota_manager_->requested_sequences() : std::set<uint32_t>{})) {
      ...
      std::string response = "{\"state\":\"" + state_val + "\","
          ...
          "\"chunk_size\":" + std::to_string(bridge_->ota_manager_ != nullptr ? bridge_->ota_manager_->chunk_size() : 0) + ","
          "\"requested\":" + requested + ","
          "\"error_msg\":\"" + error_val + "\"}";
  ...
  struct OtaChunkHandler : public AsyncWebHandler {
    ...
    bool deliver_source_chunk_(AsyncWebServerRequest *request, uint32_t seq, const std::vector<uint8_t> &chunk) {
      if (bridge_->ota_manager_ != nullptr &&
          bridge_->ota_manager_->requested_sequences().find(seq) == bridge_->ota_manager_->requested_sequences().end()) {
    ...
    void handleRequest(AsyncWebServerRequest *request) override {
      ...
      auto chunk_sz = bridge_->ota_manager_ != nullptr ? bridge_->ota_manager_->chunk_size() : 0;
      if (total == 0 || (chunk_sz > 0 && total > chunk_sz) || offset > total ||
...
void ESPNowLRBridge::register_v2_web_handlers_() {
  ...
  struct V2OtaPageHandler : public AsyncWebHandler {
    ESPNowLRBridge *bridge_;
    V2OtaPageHandler(ESPNowLRBridge *bridge) : bridge_(bridge) {}

    bool canHandle(AsyncWebServerRequest *request) const override {
      char url_buf[AsyncWebServerRequest::URL_BUF_SIZE]{};
      return request->method() == HTTP_GET && request->url_to(url_buf) == std::string("/v2/ota");
    }

    void handleRequest(AsyncWebServerRequest *request) override {
      if (!bridge_->v2_verify_session_(request)) {
        std::string redirect = "/login?redirect=/v2/ota";
        if (request->hasArg("mac")) {
          redirect += std::string("&mac=") + request->arg("mac").c_str();
        }
        request->redirect(redirect);
        return;
      }
      request->send(200, "text/html; charset=utf-8", OTA_V2_PAGE_HTML);
    }
  ...
  web_server_base::global_web_server_base->add_handler(new V2OtaPageHandler(this));
}
