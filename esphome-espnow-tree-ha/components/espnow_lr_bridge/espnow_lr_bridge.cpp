...
void ESPNowLRBridge::queue_availability_(const uint8_t *mac, bool online, const char *reason) {
  ...
  if (api_ws_ != nullptr) {
    ...
    const uint32_t offline_s =
        (session == nullptr || online) ? 0 : ((now_ms - session->last_seen_ms) / 1000U);
...
void ESPNowLRBridge::register_ota_web_handlers_() {
  ...
  struct OtaStartHandler : public AsyncWebHandler {
    ...
    void handleRequest(AsyncWebServerRequest *request) override {
      ...
      uint16_t remote_max = session ? session->session_max_payload : ESPNOW_V1_MAX_PAYLOAD;
      if (!bridge_->ota_manager_->start_transfer(mac, file_size, md5, ESPNOW_FILE_ACTION_OTA_FLASH,
                                                   remote_max, "ota.bin")) {
  ...
  struct OtaAbortHandler : public AsyncWebHandler {
    ...
    void handleRequest(AsyncWebServerRequest *request) override {
      ...
      if (bridge_->ota_manager_->is_busy()) {
        bridge_->ota_manager_->on_source_abort(ESPNOW_FILE_ABORT_USER);
      }
      bridge_->reset_ota_upload_state_(true);
