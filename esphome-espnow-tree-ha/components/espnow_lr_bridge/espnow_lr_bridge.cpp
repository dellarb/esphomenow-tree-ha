...
bool ESPNowLRBridge::add_peer_(const uint8_t *mac) {
...
uint8_t ESPNowLRBridge::current_wifi_channel_() const {
  uint8_t primary = 1;
...
void ESPNowLRBridge::queue_discovery_(const uint8_t *mac, const BridgeEntitySchema &entity, uint8_t total_entities,
  ...
  if (dev.node_id.empty()) {
    ...
    dev.node_id = dev_key;
    dev.device_id = std::string("espnow_lr_") + dev_key;
...
void ESPNowLRBridge::queue_availability_(const uint8_t *mac, bool online, const char *reason) {
...
void ESPNowLRBridge::build_entity_component_(JsonObject cmp, const uint8_t *mac, const BridgeEntitySchema &entity) {
  const auto type = static_cast<espnow_field_type_t>(entity.entity_type);
...
void ESPNowLRBridge::publish_device_discovery_(const uint8_t *mac) {
  if (!is_connected()) return;
  const std::string nk = node_key_(mac);
  ...
  const bool ok = publish_json(discovery_topic, [this, &dev, avail_topic, connection_unique_id](JsonObject root) {
    ...
    origin["name"] = "espnow-tree-bridge";
...
void ESPNowLRBridge::do_clear_device_discovery_(const uint8_t *mac) {
  if (!is_connected()) return;
  const std::string nk = node_key_(mac);
...
void ESPNowLRBridge::log_ram_stats_() {
...
std::string ESPNowLRBridge::api_topology_snapshot_json(const std::string &request_payload_json) const {
  ...
  json += "\"device_unique_id\":\"espnow_lr_" + mac_key_string_(sta_mac_.data()) + "\",";
...
std::string ESPNowLRBridge::api_node_schema_json(const std::string &mac_colon) const {
  std::string clean;
...
std::string ESPNowLRBridge::api_node_state_json(const std::string &mac_colon) const {
  std::string clean;
  for (char c : mac_colon) {
    if (c != ':' && c != '-' && c != ' ') clean += c;
  }
...
bool ESPNowLRBridge::api_node_config_start(const std::string &mac_colon, uint8_t command,
...
std::string ESPNowLRBridge::api_runtime_bridge_mac() const {
...
void ESPNowLRBridge::api_runtime_encode_auth_ok(const std::string &request_id, std::vector<uint8_t> &out) const {
  const std::string bridge_mac = mac_colon_string_(sta_mac_.data());
  const std::string device_name = sanitize_object_id(App.get_name().str());
...
void ESPNowLRBridge::api_runtime_encode_full_snapshot(const std::string &request_id, std::vector<uint8_t> &out) const {
  const std::string bridge_mac = mac_colon_string_(sta_mac_.data());
...
void ESPNowLRBridge::api_runtime_encode_bridge_heartbeat(std::vector<uint8_t> &out) const {
...
void ESPNowLRBridge::api_runtime_encode_topology_changed(const char *reason, const uint8_t *mac,
...
void ESPNowLRBridge::api_runtime_encode_remote_availability(const uint8_t *mac, bool online, const char *reason,
...
void ESPNowLRBridge::api_runtime_encode_remote_state(const uint8_t *mac, const espnow_entity_schema_t &entity,
                                                     const std::vector<uint8_t> &value, espnow_field_type_t type,
                                                     std::vector<uint8_t> &out) const {
  const BridgeSession *session = protocol_.get_session(mac);
  const std::string session_id = session == nullptr ? "" : runtime_session_id_(*session, network_id_);
...
void ESPNowLRBridge::api_runtime_encode_remote_schema_changed(const uint8_t *mac, const std::string &schema_hash,
                                                              std::vector<uint8_t> &out) const {
  const BridgeSession *session = protocol_.get_session(mac);
  const std::string bridge_mac = mac_colon_string_(sta_mac_.data());
  const std::string session_id = session == nullptr ? "" : runtime_session_id_(*session, network_id_);
...
void ESPNowLRBridge::api_runtime_handle_command(const std::string &request_id,
...
void ESPNowLRBridge::api_runtime_handle_config_command(
...
void ESPNowLRBridge::register_web_handler_() {
  ...
  struct BridgeJsonHandler : public AsyncWebHandler {
    ESPNowLRBridge *bridge_;
    explicit BridgeJsonHandler(ESPNowLRBridge *b) : bridge_(b) {}
    bool canHandle(AsyncWebServerRequest *request) const override {
      char url_buf[AsyncWebServerRequest::URL_BUF_SIZE]{};
...
void ESPNowLRBridge::register_v2_auth_handlers_() {
  if (web_server_base::global_web_server_base == nullptr) return;
  ...
  struct LoginHandler : public AsyncWebHandler {
    ESPNowLRBridge *bridge_;
    LoginHandler(ESPNowLRBridge *bridge) : bridge_(bridge) {}
  ...
  struct LogoutHandler : public AsyncWebHandler {
    ESPNowLRBridge *bridge_;
    LogoutHandler(ESPNowLRBridge *bridge) : bridge_(bridge) {}
  ...
  struct TopologyHttpHandler : public AsyncWebHandler {
    ESPNowLRBridge *bridge_;
    explicit TopologyHttpHandler(ESPNowLRBridge *b) : bridge_(b) {}
    bool canHandle(AsyncWebServerRequest *request) const override {
      char url_buf[AsyncWebServerRequest::URL_BUF_SIZE]{};
  ...
  struct BridgeInfoHttpHandler : public AsyncWebHandler {
    ESPNowLRBridge *bridge_;
    explicit BridgeInfoHttpHandler(ESPNowLRBridge *b) : bridge_(b) {}
    bool canHandle(AsyncWebServerRequest *request) const override {
      char url_buf[AsyncWebServerRequest::URL_BUF_SIZE]{};
  ...
  struct NodeStateHttpHandler : public AsyncWebHandler {
    ESPNowLRBridge *bridge_;
    explicit NodeStateHttpHandler(ESPNowLRBridge *b) : bridge_(b) {}
    bool canHandle(AsyncWebServerRequest *request) const override {
      char url_buf[AsyncWebServerRequest::URL_BUF_SIZE]{};
...
bool ESPNowLRBridge::v2_http_authenticate_(void *req) {
  if (api_key_.empty()) {
    return true;
  }
...
bool ESPNowLRBridge::v2_verify_session_(void *req) {
  if (api_key_.empty()) {
    return true;
  }
...
void ESPNowLRBridge::register_v2_web_handlers_() {
  if (web_server_base::global_web_server_base == nullptr) return;

  struct V2TopologyPageHandler : public AsyncWebHandler {
    ESPNowLRBridge *bridge_;
    V2TopologyPageHandler(ESPNowLRBridge *bridge) : bridge_(bridge) {}
