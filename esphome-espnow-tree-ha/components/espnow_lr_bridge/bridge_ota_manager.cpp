...
void ESPNowOTAManager::IncrementTracker::reset() {
  ...
  total_chunks = 0;
  buffer_size_kb = ESPNOW_LR_MAX_INCREMENT_KB;
  retransmit_round = 0;
...
void ESPNowOTAManager::loop() {
  ...
  if (tracker_.last_remote_activity_ms > 0 &&
      (now - tracker_.last_remote_activity_ms) > ESPNOW_LR_RADIO_SILENCE_ABORT_MS &&
      (state_ == State::BLASTING || state_ == State::WAITING_GAPS)) {
    fail_transfer_(ESPNOW_LR_FILE_ABORT_TIMEOUT, "radio silence from remote", true);
    return;
  }

  switch (state_) {
    case State::ANNOUNCING:
      if (control_attempt_count_ == 0 || (now - last_control_send_ms_) >= ESPNOW_LR_FILE_ANNOUNCE_TIMEOUT_MS) {
        if (!send_announce_() && state_ == State::IDLE) {
          return;
        }
      }
    ...
    case State::WAITING_GAPS:
      if (tracker_.blast_complete_sent_ms > 0 &&
          (now - tracker_.blast_complete_sent_ms) >= ESPNOW_LR_BLAST_COMPLETE_TIMEOUT_MS) {
        if (tracker_.blast_complete_retries >= ESPNOW_LR_MAX_BLAST_COMPLETE_RETRIES) {
          fail_transfer_(ESPNOW_LR_FILE_ABORT_TIMEOUT, "BLAST_COMPLETE retry limit exceeded", true);
          return;
        }
...
bool ESPNowOTAManager::send_announce_() {
  ...
  if (control_attempt_count_ >= ESPNOW_LR_FILE_MAX_RETRIES) {
  ...
  if (err == ESP_ERR_ESPNOW_NO_MEM) {
    ...
    fail_transfer_(ESPNOW_LR_FILE_ABORT_SEND_FAILURE, "announce no mem", true);
    return false;
  }
...
bool ESPNowOTAManager::send_blast_complete_() {
  ...
  bc.phase = ESPNOW_LR_FILE_PHASE_BLAST_COMPLETE;
  ...
  if (err != ESP_OK) {
    if (err == ESP_ERR_NOT_FOUND) {
      fail_transfer_(ESPNOW_LR_FILE_ABORT_SESSION_LOST, "target unreachable during BLAST_COMPLETE", false);
    }
    return false;
  }
...
bool ESPNowOTAManager::send_end_() {
  ...
  if (err == ESP_ERR_ESPNOW_NO_MEM) {
    mark_no_mem_backoff_();
    fail_transfer_(ESPNOW_LR_FILE_ABORT_SEND_FAILURE, "end no mem", true);
    return false;
  }
  clear_no_mem_backoff_();
  if (err != ESP_OK) {
...
void ESPNowOTAManager::handle_accept_(const uint8_t *trailing, size_t trailing_len) {
  ...
  if (remote_buffer_kb == 0) {
    fail_transfer_(ESPNOW_LR_FILE_ABORT_SEND_FAILURE, "remote buffer_size_kb is 0", true);
    return;
  }

  negotiated_chunk_size_ = negotiated_chunk_size;

  uint16_t effective_kb = std::min<uint16_t>(remote_buffer_kb, ESPNOW_LR_MAX_INCREMENT_KB);
...
void ESPNowOTAManager::handle_gaps_ack_(const uint8_t *trailing, size_t trailing_len) {
  if (trailing == nullptr || trailing_len < 2) {
    fail_transfer_(ESPNOW_LR_FILE_ABORT_SEND_FAILURE, "malformed gaps ack", true);
    return;
  }
  ...
  if (trailing_len < expected_trailing) {
    fail_transfer_(ESPNOW_LR_FILE_ABORT_SEND_FAILURE, "gaps ack trailing too short", true);
    return;
  }
  ...
  if (tracker_.retransmit_round >= ESPNOW_LR_MAX_RETRANSMIT_ROUNDS) {
    fail_transfer_(ESPNOW_LR_FILE_ABORT_TIMEOUT, "retransmit rounds exceeded", true);
    return;
  }
...
uint16_t ESPNowOTAManager::clamp_chunk_size_(uint16_t remote_max_frame_payload) const {
  ...
  return static_cast<uint16_t>(remote_max_frame_payload - ESPNOW_LR_FILE_DATA_HEADER_OVERHEAD);
}
