...
bool FileReceiver::handle_announce_(const espnow_file_announce_t &announce) {
  ...
  last_ack_sent_ms_ = 0;
  ESP_LOGI(TAG, "OTA announce accepted: file_size=%u chunk=%u window=%u max_chunk=%u total_chunks=%u",
           static_cast<unsigned>(file_size_), static_cast<unsigned>(chunk_size_),
           static_cast<unsigned>(window_size_), static_cast<unsigned>(max_chunk_size_),
...
bool FileReceiver::handle_file_data(const uint8_t *payload, size_t len) {
  ...
  if (acked_sequence_ == UINT32_MAX) {
    ...
    if (header->sequence <= acked_sequence_) {
      // A duplicate already-acked chunk usually means the bridge missed our
      // cumulative progress ACK. Re-send progress so the bridge can slide its
      // window instead of burning retry budget on an already-delivered chunk.
...
FileReceiver::AnnounceResponse FileReceiver::OTAFlashHandler::on_announce(uint32_t file_size, const uint8_t md5[16],
  ...
  ESP_LOGI(TAG, "OTAFlash accept: chunk_size=%u max_chunk_size=%u",
           static_cast<unsigned>(negotiated_chunk_size), static_cast<unsigned>(max_chunk_size_));
