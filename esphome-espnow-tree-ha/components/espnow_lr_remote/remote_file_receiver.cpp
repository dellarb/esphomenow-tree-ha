...
bool FileReceiver::handle_announce_(const espnow_file_announce_t &announce) {
  ...
  if (state_ != State::IDLE || (active_handler_ != nullptr && active_handler_->is_receiving())) {
    if (state_ == State::ANNOUNCED &&
      ...
      return this->send_ack_accept_(chunk_payload_size_, action_, buffer_size_kb_) ? true : false;
    }
  ...
  if (announce.file_size > 1073741824U) {
    ...
    return this->send_ack_reject_(announce.action, ESPNOW_LR_FILE_REJECT_NO_SPACE);
  }
  ...
  uint16_t remote_buffer_size_kb = ESPNOW_LR_MAX_INCREMENT_KB;
  ...
  if (total_chunks_ == 0 || total_increments_ == 0 || chunks_per_increment_ == 0) {
    active_handler_->on_abort(ESPNOW_LR_FILE_ABORT_FLASH_ERROR);
    this->reset_();
    return this->send_ack_reject_(announce.action, ESPNOW_LR_FILE_REJECT_UNSUPPORTED);
  }
...
bool FileReceiver::handle_blast_complete_(const espnow_file_blast_complete_t &bc) {
  ...
  if (retransmit_round_ >= ESPNOW_LR_MAX_RETRANSMIT_ROUNDS) {
    ESP_LOGW(TAG, "Max retransmit rounds (%u) exceeded for increment %u",
             static_cast<unsigned>(ESPNOW_LR_MAX_RETRANSMIT_ROUNDS),
...
void FileReceiver::loop() {
  ...
  const uint32_t now = millis();
  if (state_ == State::ANNOUNCED) {
    if ((now - last_activity_ms_) > ESPNOW_LR_FILE_ANNOUNCE_TIMEOUT_MS) {
  ...
  if (state_ == State::WAITING_END) {
    if ((now - last_activity_ms_) > ESPNOW_LR_RADIO_SILENCE_ABORT_MS) {
  ...
  if ((now - last_activity_ms_) > ESPNOW_LR_RADIO_SILENCE_ABORT_MS) {
    ESP_LOGW(TAG, "Aborting FILE_TRANSFER due to radio silence timeout (%ums)",
             static_cast<unsigned>(ESPNOW_LR_RADIO_SILENCE_ABORT_MS));
    this->abort(ESPNOW_LR_FILE_ABORT_TIMEOUT, true);
  }
}
...
bool FileReceiver::send_ack_(uint8_t result, const uint8_t *trailing, size_t trailing_len) {
  ...
  ack->result = result;
  ack->ref_tx_counter = (result == ESPNOW_LR_FILE_ACK_GAPS)
...
bool FileReceiver::send_gaps_ack_(const uint8_t *bitmap, size_t bitmap_len) {
  ...
  return this->send_ack_(ESPNOW_LR_FILE_ACK_GAPS, trailing.data(), trailing.size());
}
...
void FileReceiver::compute_increment_geometry_() {
  uint16_t effective_kb = std::min<uint16_t>(buffer_size_kb_, ESPNOW_LR_MAX_INCREMENT_KB);
...
void FileReceiver::reset_() {
  ...
  buffer_size_kb_ = ESPNOW_LR_MAX_INCREMENT_KB;
...
FileReceiver::AnnounceResponse FileReceiver::OTAFlashHandler::on_announce(
  ...
  if (heap_kb > ESPNOW_LR_MAX_INCREMENT_KB) {
    heap_kb = ESPNOW_LR_MAX_INCREMENT_KB;
  }
