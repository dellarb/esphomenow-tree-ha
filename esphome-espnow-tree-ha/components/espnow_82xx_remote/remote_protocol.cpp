...
// the crypto layer lives in espnow_lr_common/espnow_crypto.cpp. If a platform-
...
bool RemoteProtocol::on_espnow_frame(const uint8_t *sender_mac, const uint8_t *data, size_t len, int8_t rssi) {
  ...
  if (parent_valid_ && memcmp(sender_mac, parent_mac_.data(), 6) == 0) {
    if (parent_link_rssi_ema_ == -127) {
      ...
      parent_link_rssi_ema_ = static_cast<int8_t>(
          (rssi + (ESPNOW_LR_RSSI_EMA_ALPHA - 1) * parent_link_rssi_ema_) / ESPNOW_LR_RSSI_EMA_ALPHA);
    }
  }
  ...
  if (handle_locally && memcmp(header.leaf_mac, leaf_mac_.data(), 6) == 0) {
    ...
    if (observed_hops < hops_to_bridge_) {
...
void RemoteProtocol::loop() {
...
#if defined(ARDUINO_ARCH_ESP8266)
      (random() % (ESPNOW_LR_RETRY_JITTER_MS * 2 + 1));
#else
  ...
  if (normal_ && tx_queue_clear && parent_link_rssi_ema_ < ESPNOW_LR_PREFERRED_RSSI_FLOOR_DBM) {
  ...
  if (normal_ && tx_queue_clear && !preferred_parents_.empty()) {
    if (topology_refresh_due_ms_ == 0) {
      topology_refresh_due_ms_ = now + ESPNOW_LR_TOPOLOGY_REFRESH_INTERVAL_MS;
    }
...
bool RemoteProtocol::handle_ack_(const uint8_t *sender_mac, const espnow_frame_header_t &header, const uint8_t *payload,
  ...
  if (ack->ack_type == PKT_FILE_TRANSFER) {
    ESP_LOGD(TAG, " %s[RX ACK (File)] %02X:%02X:%02X:%02X:%02X:%02X len=%u%s", COLOR_AQUA,
             ...
             (header.hop_count & ESPNOW_LR_HOPS_V2_MTU_BIT) ? " v2" : "", COLOR_RESET);
  }
  return true;
}
...
bool RemoteProtocol::send_heartbeat_() {
  ...
  const uint32_t tx_counter = tx_counter_++;
  const bool sent = send_frame_(parent_mac_.data(), PKT_HEARTBEAT, ESPNOW_HOPS_MAKE(ESPNOW_HOPS_DIR_UP, hops_to_bridge_) | (route_v2_capable_ ? ESPNOW_LR_HOPS_V2_MTU_BIT : 0), tx_counter, reinterpret_cast<const uint8_t *>(&heartbeat), sizeof(heartbeat), true);
  if (sent) {
...
bool RemoteProtocol::send_state_(uint8_t field_index, const std::vector<uint8_t> &value, bool reset_retry_state, uint8_t retry_count) {
  ...
  for (size_t chunk = 0; chunk < chunk_count; chunk++) {
    ...
    memcpy(header.leaf_mac, leaf_mac_.data(), 6);
    header.tx_counter = tx_counter;
    header.hop_count = ESPNOW_HOPS_MAKE(ESPNOW_HOPS_DIR_UP, hops_to_bridge_) | (route_v2_capable_ ? ESPNOW_LR_HOPS_V2_MTU_BIT : 0);
    ...
    if (!send_frame_(parent_mac_.data(), PKT_STATE, ESPNOW_HOPS_MAKE(ESPNOW_HOPS_DIR_UP, hops_to_bridge_) | (route_v2_capable_ ? ESPNOW_LR_HOPS_V2_MTU_BIT : 0), tx_counter, state.data(), state.size(), true)) {
...
bool RemoteProtocol::send_command_ack_(uint8_t field_index, uint8_t result, uint32_t ref_tx_counter) {
  ...
  return send_frame_(parent_mac_.data(), PKT_ACK, ESPNOW_HOPS_MAKE(ESPNOW_HOPS_DIR_UP, hops_to_bridge_) | (route_v2_capable_ ? ESPNOW_LR_HOPS_V2_MTU_BIT : 0), tx_counter, reinterpret_cast<const uint8_t *>(&ack), sizeof(ack), true);
}
...
bool RemoteProtocol::send_ack_(const uint8_t *payload, size_t payload_len, uint32_t ref_tx_counter) {
  ...
  ESP_LOGD(TAG, " %s[TX ACK (%s)] %s len=%u%s", COLOR_AQUA, ack_label,
           ...
           route_v2_capable_ ? " v2" : "", COLOR_RESET);
  return send_frame_(parent_mac_.data(), PKT_ACK, ESPNOW_HOPS_MAKE(ESPNOW_HOPS_DIR_UP, hops_to_bridge_) | (route_v2_capable_ ? ESPNOW_LR_HOPS_V2_MTU_BIT : 0),
...
append_identity_descriptor_payload(push,
  ...
  return send_frame_(parent_mac_.data(), PKT_SCHEMA_PUSH, ESPNOW_HOPS_MAKE(ESPNOW_HOPS_DIR_UP, hops_to_bridge_) | (route_v2_capable_ ? ESPNOW_LR_HOPS_V2_MTU_BIT : 0), tx_counter,
...
bool RemoteProtocol::send_schema_push_(uint8_t entity_index) {
  ...
  for (size_t chunk = 0; chunk < chunk_count; chunk++) {
    ...
    if (!send_frame_(parent_mac_.data(), PKT_SCHEMA_PUSH, ESPNOW_HOPS_MAKE(ESPNOW_HOPS_DIR_UP, hops_to_bridge_) | (route_v2_capable_ ? ESPNOW_LR_HOPS_V2_MTU_BIT : 0), tx_counter, fragment.data(), fragment.size(), true)) {
      return false;
    }
  }
  return true;
...
#if defined(ARDUINO_ARCH_ESP8266)
      (random() % (ESPNOW_LR_DISCOVER_ANNOUNCE_JITTER_MS + 1));
#else
      esp_random() % (ESPNOW_LR_DISCOVER_ANNOUNCE_JITTER_MS + 1);
#endif
