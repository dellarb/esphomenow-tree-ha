...
class BridgeDB:
    def __init__(self, path: str | Path | None = None) -> None:
        self.path = Path(path or os.environ.get("ESP_TREE_DB", SHARED_DB_PATH))
    ...
    async def get_bridges(self) -> list[BridgeRow]:
        if not self.path.exists():
            _LOGGER.info("ESP Tree shared DB is not available yet at %s", self.path)
            return []
        try:
            ...
            _LOGGER.info("ESP Tree bridge DB read deferred: %s", exc)
            return []
    ...
    async def get_bridge(self, bridge_uuid: str) -> BridgeRow | None:
        ...
        try:
            ...
            _LOGGER.info("ESP Tree bridge DB lookup deferred: %s", exc)
            return None
        if not rows:
            return None
