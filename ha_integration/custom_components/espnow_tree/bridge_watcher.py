...
class BridgeWatcher:
    ...
    async def _remove_bridge_entry(self, entry: ConfigEntry) -> None:
        _LOGGER.info("Removing ESPNow Tree bridge entry %s because it no longer exists in the shared DB", entry.title)
        await self.hass.config_entries.async_remove(entry.entry_id)

    async def _reload_bridge_entry(self, entry: ConfigEntry, bridge: BridgeRow) -> None:
        _LOGGER.info("Reloading ESPNow Tree bridge entry %s after shared DB change", bridge.title)
