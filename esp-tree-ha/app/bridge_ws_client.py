        ...
        for node in self._topology_cache.get("nodes", []):
            if normalize_mac(node.get("mac", "")) == mac:
                if "online" in payload:
                    ...
                    if was_online and not payload["online"]:
    ...
    def get_topology_list(self) -> list[dict[str, Any]]:
        ...
        if bridge:
            ...
            result.append({
                ...
                "device_unique_id": bridge.get("device_unique_id") or ("esp_tree_" + bridge_mac_key),
                "parent_mac": "",
                "name": bridge_name,
                ...
                "model": bridge.get("model", "esp_tree_bridge"),
                ...
                "bridge_uptime_s": bridge.get("uptime_s"),
            })
        for node in nodes:
            ...
            entry: dict[str, Any] = {
                ...
                "model": identity.get("model", "esp_tree_remote"),
