...
std::string BridgeApiMessages::auth_ok(const std::string &id) {
  ...
  payload += "{\"api_version\":";
  payload += std::to_string(kApiVersion);
  payload += ",\"server\":\"esp-tree-bridge\"}";
...
std::string BridgeApiMessages::remote_availability(const uint8_t *mac, bool online, const char *reason, int8_t rssi,
                                                   ...
                                                   uint8_t hop_count) {
  std::string mac_colon = mac_display(mac);
  std::string parent_mac_colon = mac_display(parent_mac);
  std::string payload;
  payload.reserve(280);
  payload += "{\"mac\":\"";
  payload += escape_json(mac_colon);
  ...
  payload += ",\"offline_s\":";
  payload += std::to_string(offline_s);
  payload += ",\"parent_mac\":\"";
  payload += escape_json(parent_mac_colon);
  payload += "\",\"hop_count\":";
  payload += std::to_string(hop_count);
  payload += "}";
...
std::string BridgeApiMessages::node_schema_result(const std::string &id, const std::string &mac,
                                                 const std::string &schema_payload_json) {
  return envelope(type::NODE_SCHEMA_RESULT, id, schema_payload_json);
}

std::string BridgeApiMessages::node_state_result(const std::string &id, const std::string &mac,
                                                 const std::string &state_payload_json) {
  return envelope(type::NODE_STATE_RESULT, id, state_payload_json);
}

std::string BridgeApiMessages::node_state_subscribed(const std::string &id, const std::string &mac) {
  std::string payload;
  payload.reserve(64);
  payload += "{\"mac\":\"";
  payload += escape_json(mac);
  payload += "\"}";
  return envelope(type::NODE_STATE_SUBSCRIBED, id, payload);
}

std::string BridgeApiMessages::node_state_unsubscribed(const std::string &id, const std::string &mac) {
  std::string payload;
  payload.reserve(64);
  payload += "{\"mac\":\"";
  payload += escape_json(mac);
  payload += "\"}";
  return envelope(type::NODE_STATE_UNSUBSCRIBED, id, payload);
}

std::string BridgeApiMessages::node_config_result(const std::string &id, const std::string &result,
                                                  const std::string &command) {
  std::string payload;
  payload.reserve(96);
  payload += "{\"result\":\"";
  payload += escape_json(result);
  payload += "\",\"command\":\"";
  payload += escape_json(command);
  payload += "\"}";
  return envelope(type::NODE_CONFIG_RESULT, id, payload);
}
...
std::string BridgeApiMessages::ota_accepted(const std::string &id, const std::string &job_id,
  ...
  payload += std::to_string(total_chunks);
  payload += ",\"requested\":[";
  bool first = true;
...
std::string BridgeApiMessages::ota_chunk_request(const std::string &id, const std::string &job_id,
  ...
  payload.reserve(256);
  payload += "{\"session_id\":\"";
  payload += escape_json(session_id);
  payload += "\",\"request_id\":\"";
  payload += escape_json(job_id);
  payload += "\",\"sequences\":[";
  bool first = true;
  ...
  payload += "],\"progress\":{";
  payload += "\"chunks_sent\":";
  payload += std::to_string(chunks_sent);
  payload += ",\"chunks_confirmed\":";
  payload += std::to_string(chunks_confirmed);
  payload += ",\"current_increment\":";
  payload += std::to_string(current_increment);
  payload += ",\"total_increments\":";
  payload += std::to_string(total_increments);
  payload += "}}";
  return envelope(type::OTA_CHUNK_REQUEST, id, payload);
}
