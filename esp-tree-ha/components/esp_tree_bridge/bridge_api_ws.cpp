...
struct BridgeApiWsTransport::Impl {
  ...
  explicit Impl(ESPTreeBridge *bridge) : bridge(bridge), router(bridge, nullptr) {
    router.set_outbound(outbound);
    auth.set_api_key(&bridge->api_key());
    router.set_client_subscriptions_map(&client_subscriptions_);
  }
  ...
  std::map<uint32_t, NodeSubscriptions> client_subscriptions_;
  ...
  void run_socket_session(AsyncWebServerRequest *request) {
    ...
    if (httpd_req_async_handler_begin(req, &async_req) != ESP_OK || async_req == nullptr) {
      close_client(client_id);
      httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "Failed to start async websocket handler");
      return;
    }
    ...
    auto *ctx = new WsTaskContext{this, async_req, fd, client_id};
    BaseType_t task_ok = xTaskCreate(ws_task_trampoline, "bridge_api_ws", 6144, ctx, 3, nullptr);
    if (task_ok != pdPASS) {
      delete ctx;
      close_client(client_id);
  ...
  void run_socket_task(WsTaskContext *ctx) {
    send_text(ctx->client_id, BridgeApiMessages::auth_challenge(session.challenge));
    read_loop(ctx->client_id, ctx->fd);
    finish_session(ctx->client_id);
  ...
  bool send_frame(uint32_t client_id, uint8_t opcode, const uint8_t *payload, size_t len) {
    ...
    if (len > kMaxOutboundFrameBytes) {
      ESP_LOGW(TAG, "Refusing to send oversized WS frame (%zu > %zu)", len, static_cast<size_t>(kMaxOutboundFrameBytes));
      return false;
    }
  ...
  bool send_text(uint32_t client_id, const std::string &text) {
    return send_frame(client_id, 0x1, reinterpret_cast<const uint8_t *>(text.data()), text.size());
  }
  ...
  void finish_session(uint32_t client_id) {
    bool had_auth = false;
    {
      ...
      active_fd = -1;
      incoming.clear();
      router.clear_client_subscriptions_(client_id);
...
BridgeApiWsTransport::BridgeApiWsTransport(ESPTreeBridge *bridge) : impl_(new Impl(bridge)) {
  impl_->outbound = this;
...
bool BridgeApiWsTransport::send_text(uint32_t client_id, const std::string &text) { return impl_->send_text(client_id, text); }
...
void BridgeApiWsTransport::emit_event(const std::string &event_json) {
  ...
  if (client_id != 0) {
    if (!send_text(client_id, event_json)) {
      close_client(client_id);
    }
  }
}
...
void BridgeApiWsTransport::emit_remote_availability(const uint8_t *mac, bool online, const char *reason, int8_t rssi,
                                                    ...
                                                    uint8_t hop_count) {
  impl_->router.emit_remote_availability(mac, online, reason, rssi, offline_s, parent_mac, hop_count);
}
