...
4. Poll `GET /api/config` every 3 seconds.
...
The restart button calls `api.requestRestart()`. After the call, the UI enters a polling loop on `GET /api/config` with a 3-second interval. When the addon responds, it re-checks `restartRequired` and `bridgeConfigured` to determine current state.
